...
class Threads(thrdscan.ThrdScan):
    ...
    def list_orphan_threads(
        ...
        for thread in thrdscan.ThrdScan.scan_threads(context, layer_name, symbol_table):
    ...
    def _generator(self):
        """
        A simple wrapper around list_orphan_threads that reuses run from thrdscan
        """
        kernel = self.context.modules[self.config["kernel"]]

        for thread in self.list_orphan_threads(
