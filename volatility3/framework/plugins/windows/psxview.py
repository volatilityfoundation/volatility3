...
class PsXView(plugins.PluginInterface):
    ...
    def get_requirements(cls):
        return [requirements.ModuleRequirement(name="kernel", description="Windows kernel", architectures=["Intel32", "Intel64"]), 
                ...
                requirements.VersionRequirement(name="psscan", component=psscan.PsScan, version=(1, 0, 0)),
                requirements.VersionRequirement(name="thrdscan", component=thrdscan.ThrdScan, version=(1, 0, 0)),
                requirements.VersionRequirement(name="handles", component=handles.Handles, version=(1, 0, 0)),
                requirements.VersionRequirement(name="sessions", component=sessions.Sessions, version=(0, 0, 0)),
                requirements.BooleanRequirement(name="identify-expected", description="In the plugin's output, replace false with \
                                                ...
                                                so you may want to double-check the legitimacy of these processes yourself.", optional=True),
                requirements.BooleanRequirement(name="physical-offsets", description="List processes with phyiscall offsets instead of virtual offsets.", optional=True)]
    
    def proc_name_to_string(self, proc):
        return proc.ImageFileName.cast("string", max_length=proc.ImageFileName.vol.count, errors="replace")

    def is_ascii(self, str):
        return str.split('.')[0].isalnum()
    
    def filter_garbage_procs(self, proc_list):
        return [p for p in proc_list if p.is_valid() and self.is_ascii(self.proc_name_to_string(p))]
    ...
    def translate_offset(self, offset):
        ...
        try:
            offset = list(self.context.layers[layer_name].mapping(offset=offset, length=0))[0][2]
        except:
    ...
    def check_psscan(self, layer_name, symbol_table):
        res = psscan.PsScan.scan_processes(context=self.context, layer_name=layer_name, symbol_table=symbol_table)
    ...
    def check_thrdscan(self):
        ...
        for ethread in thrdscan.ThrdScan.scan_threads(self.context, module_name='kernel'):
            ...
            try:
                ...
                vollog.log(constants.LOGLEVEL_VVV, "Unable to find the owning process of ethread")
            ...
            if name == 'csrss.exe':
                try:
                    if p.has_member("ObjectTable"):
                        handles_plugin = handles.Handles(context=self.context, config_path=self.config_path)
                        hndls = list(handles_plugin.handles(p.ObjectTable))
                        for h in hndls:
                            if (h.get_object_type(handles_plugin.get_type_map(self.context, layer_name, symbol_table)) == "Process"):
                                ret.append(h.Body.cast("_EPROCESS"))

                except exceptions.InvalidAddressException:
                    vollog.log(constants.LOGLEVEL_VVV, "Cannot access eprocess object table")
    ...
    def _generator(self):
        ...
        kdbg_list_processes = list(pslist.PsList.list_processes(context=self.context, layer_name=layer_name, symbol_table=symbol_table))
        ...
        processes['csrss'] = self.check_csrss_handles(kdbg_list_processes, layer_name, symbol_table)
        ...
        for source in processes:
            for offset in processes[source]:
                if offset not in seen_offsets:
                    ...
                    in_sources = {src:str(offset in processes[src]) for src in processes}
                    ...
                    yield (0, (format_hints.Hex(offset), name, pid, in_sources["pslist"], 
                               in_sources["psscan"], in_sources["thrdscan"], in_sources["csrss"], 
                               in_sources["sessions"], exit_time))
    ...
    def run(self):
        ...
        return TreeGrid([(offset_str, format_hints.Hex), ("Name", str), ("PID", int), ("pslist", str), ("psscan", str),
                          ("thrdscan", str), ("csrss", str), ("sessions", str), ("Exit Time", str) ], self._generator())