        ...
        for driver in driverscan.DriverScan.scan_drivers(self.context, kernel.layer_name, kernel.symbol_table_name):
            try:
                try:
                    ...
                    vollog.log(constants.LOGLEVEL_VVVV, f"Failed to get Driver name : {driver.vol.offset:x}")
                    driver_name = renderers.UnparsableValue()

                yield (0, (format_hints.Hex(driver.vol.offset), "DRV", driver_name, renderers.NotApplicableValue(),
                ...
                for device in driver.get_devices():
                    try:
                        ...
                        vollog.log(constants.LOGLEVEL_VVVV, f"Failed to get Device name : {device.vol.offset:x}")
                        device_name = renderers.UnparsableValue()

                    device_type = DEVICE_CODES.get(device.DeviceType, "UNKNOWN")

                    yield (1, (format_hints.Hex(driver.vol.offset), "DEV", driver_name, device_name,
                    ...
                    for level, attached_device in enumerate(device.get_attached_devices(), start = 2):
                        ...
                        yield (level, (format_hints.Hex(driver.vol.offset), "ATT", driver_name, device_name,
                                       attached_device_driver_name, attached_device_type))
