...
class LdrModules(interfaces.plugins.PluginInterface):
    ...
    def _generator(self, procs):
        ...
        for proc in procs:
            ...
            load_order_mod = dict((mod.DllBase, mod) for mod in proc.load_order_modules())
            init_order_mod = dict((mod.DllBase, mod) for mod in proc.init_order_modules())
            ...
            for base in mapped_files.keys():
                ...
                yield (0, [
                    ...
                    str(proc.ImageFileName.cast("string", max_length = proc.ImageFileName.vol.count,
                                                errors = 'replace')),
                    format_hints.Hex(base), load_mod is not None, init_mod is not None, mem_mod is not None,
                    mapped_files[base]
                ])
    ...
    def run(self):
        ...
        return renderers.TreeGrid([("Pid", int), ("Process", str), ("Base", format_hints.Hex), ("InLoad", bool),
                                   ("InInit", bool), ("InMem", bool), ("MappedPath", str)],
                                  self._generator(
