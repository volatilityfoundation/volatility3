...
class Passphrase(interfaces.plugins.PluginInterface):
    ...
    def scan_module(self, module_base: int, layer_name: str) -> Generator[Tuple[int, str], None, None]:
        ...
        data_section: StructType = next(
            sec for sec in dos_header.get_nt_header().get_sections()
        ...
        format = DataFormatInfo(length=DWORD_SIZE_BYTES, byteorder="little", signed=True)
        int32 = ObjectTemplate(
            Integer,
            pe_table_name + constants.BANG + 'int',
            data_format=format
        )
    ...
    def _generator(self):
        ...
        truecrypt_module_base = next(
            mod.DllBase for mod in mods
            if mod.BaseDllName.get_string().lower() == 'truecrypt.sys'
        )
        for offset, password in self.scan_module(truecrypt_module_base, kernel.layer_name):
