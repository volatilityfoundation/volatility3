...
class Envars(plugins.PluginInterface):
    ...
    def _generator(self, tasks):
        ...
        for task in tasks:
            ...
            if is_64bit and task.p_sysent.sv_flags & 0x100 == 0x100 and task.p_vmspace.has_member(
                ...
                nenvstr = ps_strings.ps_nenvstr
                for i in range(nenvstr):
                    vector = struct.unpack("<I", proc_layer.read(ps_strings.ps_envstr + 4 * i, 4))[0]
                    if vector:
                        env = proc_layer.read(vector, 256)
                        envar_pair = env.split(b"\x00")[0]
                        key, value = envar_pair.decode().split("=", 1)
                        yield (0, (task_pid, task_comm, task_path, key, value))
                ...
                for i in range(nenvstr):
                    if is_64bit:
                        vector = struct.unpack("<Q", proc_layer.read(ps_strings.ps_envstr + 8 * i, 8))[0]
                    else:
                        vector = struct.unpack("<I", proc_layer.read(ps_strings.ps_envstr + 4 * i, 4))[0]
                    if vector:
                        env = proc_layer.read(vector, 256)
    ...
    def run(self):
        ...
        return renderers.TreeGrid(
            [("PID", int), ("COMM", str), ("PATHNAME", str), ("KEY", str), ("VALUE", str)],
