...
from volatility.framework.objects.utility import pointer_to_string
...
class task_struct(GenericIntelProcess):
...
class vm_area_struct(objects.Struct):
    ...
    def page_offset(self):
        ...
        return self.vm_pgoff << PAGE_SHIFT
...
class struct_file(objects.Struct):
    ...
    def full_path(self):
        ...
        while path_dentry != 0 and path_dentry.vol.offset not in seen:
            name = pointer_to_string(path_dentry.d_name.name, path_dentry.d_name.len)
            if name == "/":
                break
            parts.insert(0, name)
